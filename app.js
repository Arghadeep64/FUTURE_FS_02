  ...
  return (
    <div className="container">
      ...
      <div className="header-bar">
        <h1>Shopper’s Destination 🛍️</h1>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          {user && (
            <img
              src={user.profileImg || `https://ui-avatars.com/api/?name=${encodeURIComponent((user.name || user.email[0].toUpperCase()))}`}
              alt="Account"
              className="avatar-img"
              title={user.name || user.email}
              style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover', border: '2px solid #a78bfa' }}
            />
          )}
            ...
            <div className="settings-panel" style={{ display: "none" }}>
              ...
              {user && (
                <>
                  <button
                    className="button"
                    style={{ marginTop: '0.5rem', background: '#a78bfa', color: '#fff' }}
                  ...
                  <button
                    className="button"
                    style={{ marginTop: '0.5rem', background: '#ef4444', color: '#fff' }}
                    onClick={() => {
...
{showAddressForm && user && (
  <div style={{ position: 'fixed', top: 80, right: 30, background: '#fff', border: '1px solid #a78bfa', borderRadius: 12, padding: 20, zIndex: 1000, boxShadow: '0 2px 12px #a78bfa33', minWidth: 260 }}>
    ...
    <input
      ...
      style={{ width: 220, marginBottom: 10 }}
    />
    <button className="button" onClick={() => setShowAddressForm(false)} style={{ marginLeft: 10 }}>Close</button>
  </div>
)}
        ...
        {orders.length === 0 ? (
          ...
          orders.map((order, i) => (
            <div key={i} className="order-item">
              <p>
                <strong>{order.buyer.name}</strong> ({order.buyer.email})<br />
                📍 Address: {order.buyer.address}<br />
                💳 Payment Method: {order.buyer.payment}
              </p>
              <ul>
